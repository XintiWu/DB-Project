  ...
  try {
    ...
    if (lendRecord.to_inventory_id) {
      const updateTargetSql = `
            ...
            RETURNING qty;
        `;
      const targetRes = await client.query(updateTargetSql, [lendRecord.qty, lendRecord.to_inventory_id, lendRecord.item_id]);

      // Clean up if qty is 0 or less
      if (targetRes.rows.length > 0 && targetRes.rows[0].qty <= 0) {
        const deleteTargetSql = `
                DELETE FROM "INVENTORY_ITEMS"
                WHERE inventory_id = $1 AND item_id = $2 AND status = 'Borrowed';
             `;
        await client.query(deleteTargetSql, [lendRecord.to_inventory_id, lendRecord.item_id]);
      }
    }
